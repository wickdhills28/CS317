        ...
        for i in range(len(self.example)):
            for j in range(len(self.example)):
                ...
                if self.example[i][j] == 0:
                    ...
                    self.cur_state[i].append(
                        Variable(self.example[i][j], (i, j)))  # Add Variable object with value to cur_state
...
class Problem:
    ...
    def is_goal(self, state):
        ...
        # Inconsistent state (an unassigned variable has an empty domain ... no possible values) means INCOMPLETE
    ...
    # Returns a tuple contains the list of actions and the variable identifier
    ...
    def result(self, state, action):
        ...
        col = action[0][1]
        new_value = action[1]
        current_state[row][col].current_value = new_value

        for var in current_state[row]:
            if new_value in var.domain:
                var.domain.remove(new_value)
        ...
        for row in current_state:
            col_var = row[col]
            if new_value in col_var.domain:
                col_var.domain.remove(new_value)

                # Check for inconsistent state
                if not col_var.domain:
                    state.consistent = False
...
if __name__ == "__main__":
    print("TESTING ASAP")
    #
    # prob_obj = Problem("LatinSquares.txt", True,20)
    # p = prob_obj.problem
